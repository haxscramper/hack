...
proc hash*(f: Feature): Hash = assert false


## Returns the set of features in [features] that are enabled by
## [dependencies].
proc featuresEnabledBy(
    features: Table[string, Feature],
    dependencies: Table[string, FeatureDependency]
  ): HashSet[Feature] =

  if (features.len == 0): return

  var enabledFeatures: HashSet[Feature]
  proc enableFeature(feature: Feature) =
  ...
  for feature in features.values:
    if feature.name in dependencies and
       dependencies[feature.name].isEnabled or
       feature.onByDefault:
