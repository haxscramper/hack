...
Term _reformatTerm(Map<PackageRef, PackageLister> packageListers, Term term) {
  ...
  if (min.isNil() and max.isNil()) return term;
  return Term(
      term.package
...
Version _reformatMin(List<PackageId> versions, VersionRange range) {
  if (range.min.isNil()) return null;
  ...
  return next.isNil().not() and equalsIgnoringPreRelease(range.min, next)
      ? next
...
Pair<Version, bool> _reformatMax(List<PackageId> versions, VersionRange range) {
  if (range.max.isNil()) return null;
  if (range.includeMax) return null;
  if (range.max.isPreRelease) return null;
  if (range.min.isNil().not() and
      range.min.isPreRelease and
  ...
  return previous.isNil().not() and equalsIgnoringPreRelease(previous, range.max)
      ? Pair(previous, true)
