...
def json2keyboard(path: str) -> List[Row]:
    ...
    for row_idx, kbd_row in enumerate(model_json[1:]):
        prev = model_json[row_idx - 1]
        row: Row = Row()

        # Rotation is carried on, not incremented
        if "r" in kbd_row[0]:
            current_rot = kbd_row[0]["r"]
            # Y offset is reset after each rotation change
            current_y = 0
