...
class MainWindow extends JFrame {
  ...
  Float evalY(Float a, Float b, Float c, Float x) {
    return (float)((a * x + 3.8 * Math.tan(x)) /
                   (Math.sqrt(b * x * x + c)));
  }
  ...
  Optional<Float> getVal(char valName) {
    ...
    if (valName == 'a') {
      ...
      throw new IllegalArgumentException("Unkndon input field: " +
                                         valName);
    }
  ...
  void computeY() {
    ...
    if (a.isPresent() && b.isPresent() && c.isPresent() && x.isPresent()) {
  ...
  void computeTable() {
    var x_start =
        Misc.getFloat(in_x_start, "Некорректно введено начальное х");
    var x_final =
        Misc.getFloat(in_x_final, "Некорректно введено конечное х");
    var x_step = Misc.getFloat(in_x_step, "Некорректно введен шаг х");
    if (x_start.isPresent() && x_final.isPresent() && x_step.isPresent()) {
      var a = getVal('a');
      var b = getVal('b');
      var c = getVal('c');
  ...
  MainWindow() {
    ...
    add(Misc.setMax_WH(
        LYTBuilder.makeAnnotatedInput("a", in_a, BoxLayout.X_AXIS), 360,
        48));
    add(Misc.setMax_WH(
        LYTBuilder.makeAnnotatedInput("b", in_b, BoxLayout.X_AXIS), 360,
        48));
    add(Misc.setMax_WH(
        LYTBuilder.makeAnnotatedInput("c", in_c, BoxLayout.X_AXIS), 360,
        48));
    add(Misc.setMax_WH(
        LYTBuilder.makeAnnotatedInput("x", in_x, BoxLayout.X_AXIS), 360,
        48));

    add(compute);

    add(Misc.setMax_WH(
        LYTBuilder.makeAnnotatedInput("y = ", out_y, BoxLayout.X_AXIS),
        360, 48));
    ...
    add(LYTBuilder.makeHorizontalPanel(new ArrayList<JComponent>() {
      {
        add(Misc.setMax_WH(LYTBuilder.makeAnnotatedInput("x начальное",
                                                         in_x_start,
                                                         BoxLayout.Y_AXIS),
                           360, 48));
        add(Misc.setMax_WH(LYTBuilder.makeAnnotatedInput(
    ...
    compute.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent event) { computeY(); }
    });

    computeTable.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) { computeTable(); }
    });
  }
}
