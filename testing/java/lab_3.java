...
class MainWindow extends JFrame {
  ...
  void initUI() {
    // Create widgets
    {
      ...
      System.out.println("Updated text on a,b,c,x");
  ...
  Optional<Float> getVal(char valName) {
    ...
    if (valName == 'a') {
      ...
      throw new IllegalArgumentException("Unkndon input field: " + valName);
    }
  ...
  void computeTable() {
    var x_start = Misc.getFloat(in_x_start, "Некорректно введено начальное х");
    var x_final = Misc.getFloat(in_x_final, "Некорректно введено конечное х");
      ...
      if (a.isPresent() && b.isPresent() && c.isPresent()) {
        ...
        while (x_current <= x_final.get()) {
          res =                        //
              ...
              res + c.get() + "\t" +   //
              res + x_current + "\t" + //
              res + evalY(a.get(), b.get(), c.get(), x_current) + "\n";
        }
  ...
  MainWindow() {
    ...
    add(LYTBuilder.makeAnnotatedInput("y = ", out_y, BoxLayout.X_AXIS));

    add(computeTable);

    add(LYTBuilder.makeHorizontalPanel(new ArrayList<JComponent>() {
      {
        add(LYTBuilder.makeAnnotatedInput("x начальное", in_x_start,
                                          BoxLayout.Y_AXIS));
        add(LYTBuilder.makeAnnotatedInput("x конечное", in_x_final,
                                          BoxLayout.Y_AXIS));
        add(LYTBuilder.makeAnnotatedInput("x шаг", in_x_step,
                                          BoxLayout.Y_AXIS));
      }
    }));
