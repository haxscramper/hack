...
struct initialize {
    initialize() { Py_InitializeEx(1); }
    ~initialize() { Py_Finalize(); }
};
...
{
  public:
    enable_threads() { _state = PyEval_SaveThread(); }
    ~enable_threads() { PyEval_RestoreThread(_state); }
...
{
  public:
    restore_tstate() { _ts = PyThreadState_Get(); }
    ~restore_tstate() { PyThreadState_Swap(_ts); }

  private:
    PyThreadState* _ts;
};

// swap the current thread state with ts, restore when the object goes out
// of scope
class swap_tstate
{
  public:
    swap_tstate(PyThreadState* ts) { _ts = PyThreadState_Swap(ts); }

    ~swap_tstate() { PyThreadState_Swap(_ts); }
...
{
  public:
    // perform the necessary setup and cleanup for a new thread running
...
char program[] = R"(
import sys
print("ARGV: %s" % hasattr(sys, "argv"))
)";

char main_program[] = R"(
import sys
sys.argv = sys.argv if hasattr(sys, "argv") else ["abc"]
print("ARGV: %d" % id(sys.argv))
)";
