...
SynsetPtr parse_synset(FILE* fp, int dbase, char* word) {
    ...
    if (synptr->hereiam != loc) {
        sprintf(
            msgbuf,
            "WordNet library error: no synset at location %d\n",
            loc);
        display_message(msgbuf);
        free(synptr);
        return (NULL);
        ...
        if ((ptrtyp == HYPERPTR
            ...
            switch (realptr) {
                ...
                case ANTPTR:
                    ...
                    break;
                case PPLPTR: sprintf(prefix, "Participle of verb "); break;
                case INSTANCE: sprintf(prefix, "INSTANCE OF=> "); break;
                case INSTANCES: sprintf(prefix, "HAS INSTANCE=> "); break;
                case HASMEMBERPTR:
                ...
                case HASSTUFFPTR:
                    ...
                    break;
                case HASPARTPTR: sprintf(prefix, "   HAS PART: "); break;
                case ISMEMBERPTR: sprintf(prefix, "   MEMBER OF: "); break;
                case ISSTUFFPTR:
                    sprintf(prefix, "   SUBSTANCE OF: ");
                    break;
                case ISPARTPTR: sprintf(prefix, "   PART OF: "); break;
                default: sprintf(prefix, "=> "); break;
            }
	       ...
	       Otherwise, just print the synset pointed to. */
            ...
            if ((ptrtyp == PERTPTR || ptrtyp == PPLPTR)
                ...
                if (ptrtyp == PPLPTR) { /* adjective pointing to verb */
                    ...
                    dbase == ADV) { /* adverb pointing to adjective */
                    printsynset(
                        "      =>",
                        cursyn,
                        "\n",
                ...
                ptrtyp == ANTPTR && dbase != ADJ && synptr->pto[i] != 0) {
                sprintf(
                    tbuf,
                    " (Sense %d)\n",
                    cursyn->wnsns[synptr->pto[i] - 1]);
	       ...
	       printed in buffer so results can be truncated later. */
            ...
            if (depth) {
                ...
                traceptrs(
                    cursyn, ptrtyp, getpos(cursyn->pos), (depth + 1));
...
static void tracenomins(SynsetPtr synptr, int dbase) {
    ...
    for (i = 0; i < synptr->ptrcount; i++) {
        if ((synptr->ptrtyp[i] == DERIVATION)
            ...
            sprintf(
                prefix, "RELATED TO->(%s) ", partnames[synptr->ppos[i]]);
    ...
    for (i = 0; i < synptr->ptrcount; i++) {
        if ((synptr->ptrtyp[i] == HYPERPTR)
            ...
            if (depth) {
                depth = depthcheck(depth, cursyn);
                traceinherit(
                    cursyn, ptrbase, getpos(cursyn->pos), (depth + 1));
                free_synset(cursyn);
            } else
                free_synset(cursyn);
        }
...
void wngrep(char* word_passed, int pos) {
    ...
    if (inputfile == NULL) {
        sprintf(
            msgbuf,
            "WordNet library error: Can't perform compounds "
...
char* findtheinfo(char* searchstr, int dbase, int ptrtyp, int whichsense) {
    ...
    switch (ptrtyp) {
        case OVERVIEW: WNOverview(searchstr, dbase); break;
        case FREQ:
            ...
            break;
        case WNGREP: wngrep(searchstr, dbase); break;
        case RELATIVES:
        case VERBGROUP:
            while ((idx = getindex(searchstr, dbase)) != NULL) {
                searchstr                                = NULL;
        ...
        default:
	       ...
	       database and perform the search requested. */
		       ...
		       with a different spelling. If so, skip it. */
                        for (i = 0, skipit = 0; i < offsetcnt && !skipit;
                             i++) {
                            ...
                            switch (ptrtyp) {
                                case ANTPTR:
                                    if (dbase == ADJ)
                                        ...
                                        traceptrs(
                                            cursyn, ANTPTR, dbase, depth);
                                    break;

                                case COORDS:
                                    tracecoords(
                                        cursyn, HYPOPTR, dbase, depth);
                                    break;

                                case FRAMES: printframe(cursyn, 1); break;
                                ...
                                case MERONYM:
                                    traceptrs(
                                        ...
                                        depth);
                                    traceptrs(
                                        cursyn, HASSTUFFPTR, dbase, depth);
                                    traceptrs(
                                        cursyn, HASPARTPTR, dbase, depth);
                                    break;

                                case HOLONYM:
                                    traceptrs(
                                        cursyn, ISMEMBERPTR, dbase, depth);
                                    traceptrs(
                                        cursyn, ISSTUFFPTR, dbase, depth);
                                    traceptrs(
                                        cursyn, ISPARTPTR, dbase, depth);
                                    break;
                                ...
                                case PPLPTR:
                                    traceptrs(
                                        cursyn, ptrtyp, dbase, depth);
                                    traceptrs(
                                        cursyn, PPLPTR, dbase, depth);
                                    break;
#endif
                    ...
                    if (skipit != 1) {
                        interface_doevents();
                        if ((whichsense == sense + 1) || abortsearch
                            || overflag)
	   ...
	   database and perform the search requested. */
        ...
        for (sense = 0; sense < idx->off_cnt; sense++) {
            if (whichsense == ALLSENSES || whichsense == sense + 1) {
                ...
                cursyn->ptrlist    = traceptrs_ds(
                    cursyn, ptrtyp, getpos(cursyn->pos), depth);
...
unsigned int in_wn(char* word, int pos) {
    ...
    if (pos == ALL_POS) {
        for (i = 1; i < NUMPARTS + 1; i++)
            if (indexfps[i] != NULL
                && bin_search(word, indexfps[i]) != NULL)
                retval |= bit(i);
    } else if (
        indexfps[pos] != NULL && bin_search(word, indexfps[pos]) != NULL)
        retval |= bit(pos);
    return (retval);
}
