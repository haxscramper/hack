...
//
//   fuzzy_match_simple(...)
//     Returns true if each character in pattern is found sequentially
//     within str
//
//   fuzzy_match(...)
//     Returns true if pattern is found AND calculates a score.
//     Performs exhaustive search via recursion to find all possible
//     matches and match with highest score. Scores values have no
//     intrinsic meaning. Possible score range is not normalized and varies
//     with pattern. Recursion is limited internally (default=10) to
//     prevent degenerate cases (pattern="aaaaaa"
//     str="aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa") Uses uint8_t for match
//     indices. Therefore patterns are limited to 256 characters. Score
//     system should be tuned for YOUR use case. Words, sentences, file
//     names, or method names all prefer different tuning.
