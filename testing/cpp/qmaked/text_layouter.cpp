...
const int infty = 1024 * 1024 * 1024;

bool inf(int a) { return (infty - 4096 <= a) and (a <= infty + 4096); }

template <typename T>
vector<T> get(const vector<optional<T>>& seq) {
    vector<T> result;
    for (const auto& it : seq) {
        if (it.has_value()) { result.push_back(it.get()); }
    }
    return result;
}

template <typename T1, typename T2, typename T3, typename T4, typenmae T5>
vector<tuple<T1, T2, T3, T4, T5>> zip(
    const vector<T1>& s1,
    const vector<T2>& s2,
    const vector<T3>& s3,
    const vector<T4>& s4,
    const vector<T5>& s5) {
    vector<tuple<T1, T2, T3, T4, T5>> result;
    int                               max = min(
        s1.size(),
        min(s2.size(), min(s3.size(), min(s4.size(), s5.size()))));

    for (int i = 0; i < max; ++i) {
        result.push_back({s1[i], s2[i], s3[i], s4[i], s5[i]});
    }
    return result;
}

#define ID_TYPE_BASIC(Name)                                               \
  private:                                                                \
    unsigned long long int id;                                            \
                                                                          \
  public:                                                                 \
    static inline Name Empty() { return Name{0}; }                        \
    bool operator==(const Name& other) const { return other.id == id; }   \
    bool isNil() const { return id == 0; }                                \
    Name(unsigned long long int value) : id(value) {}                     \
    unsigned long long int get() const { return id; }

// #define ID_TYPE_MASKING(Name) \
//  public: \
...
struct LytStrId {
    ID_TYPE_BASIC(LytStrId);
};

// declareIdType(LytStr) declareHighMasking(LytStr, highMaskRange = 8)
...
struct LayoutElementObj;
using LayoutElement = shared_ptr<LayoutElementObj>;

struct LayoutObj {
    vector<LayoutElement> elements;
};

using Layout = shared_ptr<LayoutObj>;
...
struct LytSolutionObj;

struct LytSolution = shared_ptr<LytSolutionObj>;
...
struct LytBlockObj;
using LytBlock = shared_ptr<LytBlockObj>;

struct LytBlockObj {
    unordered_map<optional<LytSolution>, optional<LytSolution>>
                 layoutCache;
    bool         isBreaking;
    int          breakMult;
    LytBlockKind kind;
    union data {
        ...
        struct _bkWrap {
            optional<LytStr> prefix;
            LytStr           sep;
            vector<LytBlock> wrapElements;
        };

        struct _bkMultiKinds {
            vector<LytBlock> elements;
        };
    };
};

struct LytFormatPolicy {
    function<vector<vector<LytBlock>>(const vector<vector<LytBlock>>& blc)>
        breakElementLines;
};
...
struct LytOptions {
    int             leftMargin;
    int             rightMargin;
    float           leftMarginCost;
    float           rightMarginCost;
    int             linebreakCost;
    float           cpack;
