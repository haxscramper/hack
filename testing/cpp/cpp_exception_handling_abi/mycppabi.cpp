...
struct Call_Site {
    ...
    // Note start, len and lp would be void*'s, but they are actually
    // relative addresses: start and lp are relative to the start of the
    // function, len is relative to start
     ...
     */
    bool valid_for_throw_ip(uintptr_t func_start, uintptr_t throw_ip)
        const {
...
struct LSDA {
    ...
    LSDA(LSDA_ptr raw_lsda)
        ...
        types_table_start(
            (const void**)(raw_lsda + header.type_table_offset))
        ,
    ...
    const Call_Site* next_call_site_entry(bool start = false) {
        ...
        if (next_cs_entry_ptr >= cs_table_end) return NULL;

        // Copy the call site table and advance the cursor by
        // sizeof(Call_Site). We need to copy the struct here because there
        // might be alignment issues otherwise
...
bool can_handle(
    const std::type_info* thrown_exception,
    ...
    if (thrown_exception->name() == catch_type->name()) { return true; }
...
_Unwind_Reason_Code __gxx_personality_v0(
    ...
    __cxa_exception*
        exception_header = (__cxa_exception*)(unwind_exception + 1) - 1;
    std::type_info* thrown_exception_type = exception_header
                                                ->exceptionType;
    ...
    for (const Call_Site* cs = lsda.next_call_site_entry(true); cs != NULL;
        ...
        for (const LSDA::Action* action = lsda.get_first_action_for_cs(cs);
            ...
            if (action->type_index == 0) {
                ...
                //
                // Of course the cleanup should only run on the cleanup
                // phase
                if (actions & _UA_CLEANUP_PHASE) {
                    return run_landing_pad(
                        unwind_exception,
                        context,
                        action->type_index,
                        func_start + cs->lp);
                ...
                const std::type_info* catch_type = lsda.get_type_for(
                    action);

                if (can_handle(thrown_exception_type, catch_type)) {
                    // If we are on search phase, tell _Unwind_ we can
                    // handle this one
                    if (actions & _UA_SEARCH_PHASE)
                        return _URC_HANDLER_FOUND;

                    // If we are not on search phase then we are on
                    // _UA_CLEANUP_PHASE and we need to install the context
                    return run_landing_pad(
                        unwind_exception,
                        context,
                        action->type_index,
