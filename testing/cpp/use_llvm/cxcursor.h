...
namespace cxcursor {
    ...
    std::pair<const ObjCInterfaceDecl*, SourceLocation> getCursorObjCSuperClassRef(
        CXCursor C);

    /// Create an Objective-C protocol reference at the given location.
    CXCursor MakeCursorObjCProtocolRef(
    ...
    /// and optionally the location where the reference occurred.
    std::pair<const ObjCProtocolDecl*, SourceLocation> getCursorObjCProtocolRef(
        CXCursor C);

    /// Create an Objective-C class reference at the given location.
    CXCursor MakeCursorObjCClassRef(
    ...
    /// and optionally the location where the reference occurred.
    std::pair<const ObjCInterfaceDecl*, SourceLocation> getCursorObjCClassRef(
        CXCursor C);

    /// Create a type reference at the given location.
    CXCursor MakeCursorTypeRef(
        const TypeDecl*   Type,
        SourceLocation    Loc,
        CXTranslationUnit TU);
    ...
    /// and optionally the location where the reference occurred.
    std::pair<const TypeDecl*, SourceLocation> getCursorTypeRef(
        CXCursor C);

    /// Create a reference to a template at the given location.
    CXCursor MakeCursorTemplateRef(
    ...
    std::pair<const TemplateDecl*, SourceLocation> getCursorTemplateRef(
        CXCursor C);

    /// Create a reference to a namespace or namespace alias at the given
    /// location.
    CXCursor MakeCursorNamespaceRef(
        const NamedDecl*  NS,
        SourceLocation    Loc,
        CXTranslationUnit TU);
    ...
    std::pair<const NamedDecl*, SourceLocation> getCursorNamespaceRef(
        CXCursor C);

    /// Create a reference to a variable at the given location.
    CXCursor MakeCursorVariableRef(
        const VarDecl*    Var,
        SourceLocation    Loc,
        CXTranslationUnit TU);
    ...
    std::pair<const VarDecl*, SourceLocation> getCursorVariableRef(
        CXCursor C);

    /// Create a reference to a field at the given location.
    CXCursor MakeCursorMemberRef(
    ...
    std::pair<const FieldDecl*, SourceLocation> getCursorMemberRef(
        CXCursor C);
    ...
    CXCursor MakePreprocessingDirectiveCursor(
        SourceRange       Range,
        CXTranslationUnit TU);

    /// Unpack a given preprocessing directive to retrieve its source
    /// range.
    SourceRange getCursorPreprocessingDirective(CXCursor C);

    /// Create a macro definition cursor.
    CXCursor MakeMacroDefinitionCursor(
    ...
    CXCursor MakeMacroExpansionCursor(
        MacroExpansion*,
        CXTranslationUnit TU);
    ...
    ///
    /// "Pseudo" macro expansion cursors (essentially a macro definition
    /// along with a source location) are created in special cases, for
    /// example they can be created for identifiers inside macro
    /// definitions, if these identifiers are macro names.
    ...
    static inline MacroExpansionCursor getCursorMacroExpansion(
        CXCursor C) {
        return C;
    }

    /// Create an inclusion directive cursor.
    CXCursor MakeInclusionDirectiveCursor(
        InclusionDirective*,
        CXTranslationUnit TU);
    ...
    std::pair<const LabelStmt*, SourceLocation> getCursorLabelRef(
        CXCursor C);

    /// Create a overloaded declaration reference cursor for an expression.
    CXCursor MakeCursorOverloadedDeclRef(
    ...
    /// Create a overloaded declaration reference cursor for a template
    /// name.
    CXCursor MakeCursorOverloadedDeclRef(
    ...
    std::pair<OverloadedDeclRefStorage, SourceLocation> getCursorOverloadedDeclRef(
        CXCursor C);
    ...
    /// Returns a index/location pair for a selector identifier if the
    /// cursor points to one.
    std::pair<int, SourceLocation> getSelectorIdentifierIndexAndLoc(
        CXCursor);
    static inline int getSelectorIdentifierIndex(CXCursor cursor) {
        return getSelectorIdentifierIndexAndLoc(cursor).first;
    }
    static inline SourceLocation getSelectorIdentifierLoc(
        CXCursor cursor) {
    ...
    /// \returns true if no USR was computed or the result should be
