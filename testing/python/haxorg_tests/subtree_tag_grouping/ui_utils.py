...
def location_line(value: Any) -> Optional[int]:
    location = getattr(value, "loc", None)
...
def location_column(value: Any) -> Optional[int]:
    location = getattr(value, "loc", None)
    return location.column if location is not None else None
...
def append_nodes(
    ...
    _, value = betterproto2.which_one_of(node, "kind")
    ...
    match value:
        ...
        case proto.Subtree():
            summary = extract_subtree_summary(value, now)
            line = location_line(value)
            column = location_column(value)
...
@beartype
def collect_subtrees(root: TreeNode) -> list[TreeNode]:
...
@beartype
def parse_tag_query(text: str) -> list[tuple[str, ...]]:
...
@beartype
def subtree_matches_tags(node: TreeNode, query: str) -> bool:
    required = parse_tag_query(query)
    if not required:
        return True
    ...
    available = [tuple(path) for path in node.summary.tags]

    return all(
        any(
            len(tag_path) >= len(required_path)
            and tag_path[:len(required_path)] == required_path
            for tag_path in available) for required_path in required)


@beartype
def cell_value(node: TreeNode, column: int) -> Any:
...
class OrgTreeModel(QAbstractItemModel):
    ...
    def data(
        ...
        return cell_value(self.node_at(index), index.column())
...
class FlatSubtreeModel(QAbstractItemModel):
    ...
    @beartype
    def replace_nodes(self, nodes: list[TreeNode]) -> None:
        self.beginResetModel()
        self.nodes = nodes
        self.endResetModel()


class OrgItemView(QTreeView):
...
@beartype
def configure_header(view: QTreeView) -> None:
    header = view.header()
...
class OrgWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.settings = QSettings(
            "haxscramper",
            "haxorg-subtree-viewers",
        )
        self.settings.beginGroup(f"documents/{document_key}/{settings_prefix}")
...
@beartype
def export_output_path(input_path: Path, viewer_name: str) -> Path:
    digest = hashlib.sha256(str(input_path.resolve()).encode()).hexdigest()
    return Path(f"/tmp/haxorg-{viewer_name}-{digest}.pb")
