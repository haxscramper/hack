from dynamic_window_view.atspi_walker import get_x11_windows, populate_splits, print_split_tree, WindowInfo, TagInfo, Rect
...
from dynamic_window_view.atspi_walker import populate_splits
...
def main():
    ...
    for tag in tags:
        ...
        print(f"  Screen: {tag.screen}")
        print(f"  Selected: {tag.selected}")
        print(f"  Activated: {tag.activated}")
        print(f"  Index: {tag.index}")
        print(f"  Layout: {tag.layout}")
        print(f"  Master count: {tag.master_count}")
        print(f"  Column count: {tag.column_count}")
        print(f"  Master width factor: {tag.master_width_factor}")
        print(f"  Gap: {tag.gap}")
        print(f"  Volatile: {tag.volatile}")
        print(f"  Windows: {len(tag.windows)}")
        ...
        for win in tag.windows:
            ...
            print(f"      PID: {win.pid}")
            print(f"      Screen: {win.screen}")
            print(f"      Tags: {', '.join(win.tags)}")
            print(
            ...
            if win.splits:
                print("      Splits:")
                print_split_tree(win.splits, indent=8)
            else:
                print("      Splits: (none detected)")
        print()
