...
def mask_by_pref(p):
    ...
    print("MASK: " + '.'.join(mask) + " в двоичной: " +
          '.'.join(ip_to_bin(mask)))
    return mask
...
while (ev != 0):
    ...
    if ev == 3:
        ...
        print("\nIP: " + '.'.join(ip) + " в двоичной: " +
              '.'.join(ip_to_bin(ip)))
        print("MASK: " + '.'.join(mask) + " в двоичной: " +
              '.'.join(ip_to_bin(mask)))
        print("СЕТ. АДР.: " + '.'.join(net_adr) + " в двоичной: " +
    ...
    if ev == 5:
        ...
        broad_adr = broad_by_mask(ip, mask)
        print("Широковещательный IPv4-адрес в этой подсети: " +
              '.'.join(broad_adr))
        last_adr = last_by_broad(broad_adr)
        print("IPv4-адрес последнего узла в этой подсети: " +
              '.'.join(last_adr))
