...
class Repository:
    ...
    def upsert_image(self, root_dir: Path, file_path: Path) -> ImageEntry:
        ...
        relative_path = str(file_path.resolve().relative_to(
            root_dir.resolve()))
    ...
    def get_fully_annotated_paths(self) -> set[str]:
        stmt = (select(ImageEntry.relative_path).join(
            ImageProbabilisticTag,
            ImageEntry.id == ImageProbabilisticTag.image_id).join(
                ImageRegularTag,
                ImageEntry.id == ImageRegularTag.image_id).join(
                    ImageDescription, ImageEntry.id == ImageDescription.
                    image_id).where(ImageDescription.description != "").where(
    ...
    def set_probabilistic_tag(self,
                              image_id: int,
                              tag_name: str,
                              probability: float,
                              category: str = "user"):
        ...
        rel = self.session.scalar(
            select(ImageProbabilisticTag).where(
                ImageProbabilisticTag.image_id == image_id).where(
                    ImageProbabilisticTag.tag_id == tag.id))
        if rel is None:
    ...
    def get_or_create_regular_tag(self, category: str,
                                  name: str) -> RegularTag:
        tag = self.session.scalar(
            select(RegularTag).where(RegularTag.category == category).where(
                RegularTag.name == name))
        if tag is None:
    ...
    def add_regular_tag(self, image_id: int, category: str, name: str):
        ...
        rel = self.session.scalar(
            select(ImageRegularTag).where(
                ImageRegularTag.image_id == image_id).where(
                    ImageRegularTag.tag_id == tag.id))
        if rel is None:
    ...
    def replace_probabilistic_annotations(
        ...
        self.session.query(ImageProbabilisticTag).filter_by(
            image_id=image_id).delete()
        self.session.commit()
        ...
        for category, name, probability in items:
            ...
            tag = self.get_or_create_probabilistic_tag(name=name,
                                                       category=category)
            self.session.add(
                ImageProbabilisticTag(
                    image_id=image_id,
    ...
    def replace_regular_annotations(
        ...
        self.session.query(ImageRegularTag).filter_by(
            image_id=image_id).delete()
        self.session.commit()
