...
def page_has_selectable_text(pdf_path: Path, page_num: int) -> bool:
...
def extract_native_page_structure(pdf_path: Path, page_num: int) -> List[DocTag]:
    """
    ...
    with pdfplumber.open(str(pdf_path)) as pdf:
        ...
        classified_lines = [_classify_line(line, page_width, page_height) for line in lines]
        ...
        for table_data in tables:
            ...
            child_tag = DocTag(
                ...
                tag_name="table",
                text=table_text,
                bbox=None,  # pdfplumber table bbox would require table.bbox from find_tables
            )
            root_tag.children.append(child_tag)
            tag_counter += 1
...
def _merge_line_words(words: List[dict]) -> dict:
    ...
    primary_font = max(set(fontnames), key=fontnames.count) if fontnames else ""
...
def _is_numbered_list_item(text: str) -> bool:
    ...
    import re
    return bool(re.match(r"^\s*(\d{1,3}[.)]\s|[a-zA-Z][.)]\s|\([a-zA-Z0-9]+\)\s)", text))
    ...
    for i, page_img in enumerate(all_pages):
        ...
        try:
            if json_output_file.exists():
                ...
                logger.info(f"{stats_header} | Page {current_page_num}: Native text detected, extracting structure directly")
                spatial_tags = extract_native_page_structure(pdf_path, current_page_num)
