...
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QGraphicsView, QGraphicsScene, QGraphicsPixmapItem, QGraphicsRectItem, QGraphicsItem, QCheckBox, QSlider, QLabel, QLineEdit
...
class CenterPanel(QWidget):
    ...
    def on_context_menu(self, pos) -> None:
        ...
        if action == mark_removed_action:
            ...
            first_item = next((item for item in selected_items
                               if isinstance(item, QGraphicsRectItem)), None)
            if first_item:
                tag = first_item.data(Qt.ItemDataRole.UserRole)
                if tag:
                    current_text = tag.user_edited_text if getattr(
                        tag, 'user_edited_text',
                        None) is not None else tag.text
            ...
            if first_item:
                tag = first_item.data(Qt.ItemDataRole.UserRole)
                if tag:
                    current_tag = tag.user_tag_override if getattr(
                        tag, 'user_tag_override', None) else tag.tag_name
            ...
            if isinstance(item, QGraphicsRectItem):
                ...
                if tag and page_meta:
                    ...
                    if tag.user_removed:
                        ...
                        pen_color = TAG_COLORS.get(tag.tag_name,
                                                   QColor(255, 0, 0, 150))
                        pen = QPen(pen_color)
                        pen.setWidth(2)
                        item.setPen(pen)
    ...
    def save_modified_pages(self, modified_pages: dict) -> None:
        for json_path, page_data in modified_pages.items():
            try:
                ...
                logging.info(
                    f"CenterPanel: Saved updated annotations to {json_path.name}"
                )
    ...
    def load_pdf(self, pdf_path: Path, output_dir: Path) -> None:
        ...
        if total > 0:
            ...
            self.slider.setLowerValue(
                5)  # skip first few pages that contain title etc.
    ...
    def _create_transparent_pixmap(self, image_path: str) -> QPixmap:
        ...
        image = QImage(image_path).convertToFormat(
        ...
        for p_idx in range(1, total + 1):
            ...
            try:
                with open(json_path, 'r') as f:
                    ...
                    pixmap = self._create_transparent_pixmap(
                        p_data.image_cache_path)
                    tag_items = []
                    ...
                    if not pixmap.isNull():
                        ...
                        tag_items = self.draw_spatial_tags(
                            p_data.spatial_tags, float(pixmap.width()),
                            float(pixmap.height()), p_data, json_path)
            ...
            if json_path.exists():
                try:
                    with open(json_path, 'r') as f:
                        ...
                        logging.info(
                            f"CenterPanel: Loaded page {self.current_page_idx}"
                        )
                        self.load_page(self.page_data.image_cache_path,
                                       self.page_data.spatial_tags, json_path)
                except Exception as e:
                ...
                self.page_data = None
                self.scene.addText(
                    f"Page {self.current_page_idx} not processed yet.")

    def load_page(self, image_path: str, spatial_tags: List[DocTag],
                  json_path: Path) -> None:
        pixmap = QPixmap(image_path)
        if not pixmap.isNull():
            pixmap_item = QGraphicsPixmapItem(pixmap)
            self.scene.addItem(pixmap_item)
            self.draw_spatial_tags(spatial_tags, float(pixmap.width()),
                                   float(pixmap.height()), self.page_data,
                                   json_path)
    ...
    def draw_spatial_tags(self, spatial_tags: List[DocTag], p_width: float,
                          p_height: float, page_data: Optional[PageData],
                          json_path: Path) -> List[QGraphicsRectItem]:
        ...
        def draw_tags(tags: List[DocTag]) -> None:
            for tag in tags:
                if tag.bbox:
                    ...
                    rect_item.setData(Qt.ItemDataRole.UserRole + 1,
                                      (page_data, json_path))
