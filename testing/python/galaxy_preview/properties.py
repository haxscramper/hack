...
class PropertiesPanel(QtWidgets.QWidget):
    ...
    def _add_hms_dms_fields(self, entry, is_relative=False):
        ...
        ra_h_spin.setValue(loc.ra.h)
        ra_h_spin.setSuffix("h ")
        ra_h_spin.valueChanged.connect(lambda v: setattr(loc.ra, 'h', v))
        ...
        ra_m_spin.setValue(loc.ra.m)
        ra_m_spin.setSuffix("m ")
        ra_m_spin.valueChanged.connect(lambda v: setattr(loc.ra, 'm', v))
        ...
        ra_s_spin.setValue(loc.ra.s)
        ra_s_spin.setSuffix("s")
        ra_s_spin.valueChanged.connect(lambda v: setattr(loc.ra, 's', v))
        ra_s_spin.valueChanged.connect(
            lambda _: self.entry_changed.emit(self.current_entry.id)
        )
        ra_layout.addWidget(ra_s_spin)

        self.specific_layout.addRow("RA:", ra_layout)

        dec_layout = QtWidgets.QHBoxLayout()
        dec_d_spin = QtWidgets.QSpinBox()
        dec_d_spin.setRange(-90, 90)
        dec_d_spin.setValue(loc.dec.d)
        dec_d_spin.setSuffix("° ")
        dec_d_spin.valueChanged.connect(lambda v: setattr(loc.dec, 'd', v))
        ...
        dec_m_spin.setValue(loc.dec.m)
        dec_m_spin.setSuffix("' ")
        dec_m_spin.valueChanged.connect(lambda v: setattr(loc.dec, 'm', v))
        ...
        dec_s_spin.setValue(loc.dec.s)
        dec_s_spin.setSuffix('"')
        dec_s_spin.valueChanged.connect(lambda v: setattr(loc.dec, 's', v))
        dec_s_spin.valueChanged.connect(
            lambda _: self.entry_changed.emit(self.current_entry.id)
        )
        dec_layout.addWidget(dec_s_spin)

        self.specific_layout.addRow("Dec:", dec_layout)

        dist_spin = QtWidgets.QDoubleSpinBox()
        dist_spin.setRange(0.0, 1000000.0)
        dist_spin.setDecimals(2)
        dist_spin.setSingleStep(1.0)
        dist_spin.setValue(loc.distance)
        dist_spin.setSuffix(" pc")
        dist_spin.valueChanged.connect(lambda v: setattr(loc, 'distance', v))
        dist_spin.valueChanged.connect(
            lambda _: self.entry_changed.emit(self.current_entry.id)
        )
        self.specific_layout.addRow("Dist:", dist_spin)
    ...
    def set_entry(self, entry: GalacticEntry, galactic_map: GalacticMap | None = None):
        ...
        if isinstance(entry, Star):
            self._add_hms_dms_fields(entry)
