...
func newCfgAst(kind: CfgAstKind, subnodes: varargs[CfgAst]): CfgAst =
  result = CfgAst(kind: kind)
  if 0 < len(subnodes):
    result.subnodes = @subnodes

func newCfgAst(kind: CfgAstKind, token: Token): CfgAst =
  result = CfgAst(kind: kind)
  result.token = token
...
proc advance(c: var CfgParser) {.gcsafe.}
...
proc parse(c: var CfgParser): CfgAst

proc parseExpr(c: var CfgParser): CfgAst =
  var tokens: seq[Token]

  while c.tok.kind != tkColon:
    tokens.add c.tok
    advance(c)
